...
class Jeu:
    ...
    def update(self):
        ...
        print(pyxel.pget(self.pacman["x"], self.pacman["y"]))
        #collision
        if  pyxel.pget(self.pacman["x"], self.pacman["y"]) == 8:# \
#            or pyxel.pget(self.pacman["x"] + LCARRE - 1, self.pacman["y"]) == 8 \
#            or pyxel.pget(self.pacman["x"], self.pacman["y"] + LCARRE - 1) == 8 \
#            or pyxel.pget(self.pacman["x"] + LCARRE - 1, self.pacman["y"] + LCARRE - 1) == 8:
            self.pacman["x"] -= dx
            self.pacman["y"] -= dy
...
#         print (x,y)
        ...
        if self.pacman["x"] <= -3 and self.pacman["y"] in range (56,69):
            self.pacman["x"] = 127
        if self.pacman["x"] >= 128 and self.pacman["y"] in range (56,69):
            self.pacman["y"] = -2
